// This program calculates the direct distance from Delta
// College to other post offices across Michigan using zip
...
const double DELTA_LON = 83.99430;    // Delta College longitude

void getData(ifstream& inFile, int zip[], double lat[],
             double lon[], int& numVals);
...
int main()
{
    ...
    fileIn.open("zipMI.txt");   // Open file
    ...
    getData(fileIn,zip,latitude,longitude,numElems);
    ...
    if (index >= 0)
    {
        ...
        cout << "post office with zip code: "
             << targetZip << " is " << endl;
        cout << distance << " miles" << endl << endl;
    }
    else
        cout << endl << "Error: Zip code not found in Michigan" << endl << endl;
             ...
             double lon[], int& numVals)
{
    ...
    inFile >> zip[i] >> lat[i] >> lon[i];     // Priming read              
    while (!inFile.eof() && i < MAX_ARRAY)        
    {
       i++;        
       inFile >> zip[i] >> lat[i] >> lon[i];  // Continuation read             
    }            
    numVals = i;
}
